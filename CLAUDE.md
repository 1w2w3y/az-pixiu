...
Phase 1 ("minimum viable agent") is complete; the project is in **Phase 2 — Langfuse depth** (see `docs/roadmap.md`). The agent runs end-to-end against live AMG-MCP and Azure AI Foundry, produces a markdown report plus a `run.json` artefact per invocation, lands a Langfuse trace for every run, and has a first evaluation dataset (`eval/phase-1.json`, 4 items) plus six scoring rubrics (structural correctness, citation completeness, confidence consistency, read-only adherence, and the Phase 3 additions `estimated_impact_calibrated` and `waste_classification_grounding`). The next concrete Phase 2 work is wiring eval results back to Langfuse as Scores, migrating prompts to Langfuse Prompts, and standing up Langfuse Datasets / Experiments. **Phase 2.5 — cross-run continuity foundations** has shipped: every `pixiu analyze` run consults a filesystem-backed `RunHistoryStore` over the local `runs/` directory, injects a synthetic `prior_run_context` evidence record when matching prior runs exist, and surfaces transport- and freshness-level findings under a top-level "Run Quality" report section; recommendations carry a deterministic `recommendation_signature` for cross-run identity. The reasoner.v1 prompt accepts but does not yet act on the prior-run context. **Phase 3 — cost-summary analyzer breadth** is partially shipped: the first waste lane (orphan public IPs with calibrated weekly impact estimates) is live, and `reasoner.v2` has shipped and is loaded for `analysis_type=cost_summary`. The reasoner-level handling of `prior_run_context` is still pending — tracked under a follow-up Phase 3 PR. The broader Phase 3 plan (additional waste lanes, naming-pattern clustering, recurring-pattern recall) is designed in [`docs/design/cost-summary-depth.md`](docs/design/cost-summary-depth.md) and only partially implemented.
