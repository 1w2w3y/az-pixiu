...
Phase 1 ("minimum viable agent") is complete; the project is in **Phase 2 — Langfuse depth** with **Phase 2.5 shipped** and the first **Phase 3 cost-summary slice** live (see `docs/roadmap.md`). The agent runs end-to-end against live AMG-MCP and Azure AI Foundry or LiteLLM, produces Markdown, HTML, and `run.json` artefacts per invocation, and can land Langfuse traces when configured. The evaluation surface has the Phase 1 dataset (`eval/phase-1.json`, 4 items), a Phase 3 waste dataset (`eval/phase-3-waste.json`, 1 item), the two-item Phase 3 cost-judgment dataset (`eval/phase-3-cost-reasoning.json`), six active automated rubrics (the four Phase 1 rubrics plus `estimated_impact_calibrated` and `waste_classification_grounding`), scenario-specific semantic expectations, and Langfuse score / Dataset Run / Experiment publishing for eval runs when credentials are present.

Current shipped foundations: `pixiu analyze` uses a filesystem-backed `RunHistoryStore`, injects `prior_run_context` evidence when matching prior runs exist, records `recommendation_signature`, writes transport summaries, surfaces Run Quality and deterministic coverage disclosure, retries retriable evidence calls with capped backoff and pacing, and detects payload-embedded `amgmcp_cost_analysis` 429/auth/authz errors. The July 2026 evidence-contract gate is also live across both deterministic playbooks and the first waste lane: Cost Analysis uses `subscriptionId/startTime/endTime`, Activity Log uses ARM `scope/startTime/endTime`, ARG scope is normalized into supported KQL with out-of-band request provenance, and MCP `isError` results fail before domain parsing. The lane decodes real MCP text envelopes and validates returned subscription, resource group, ARM type, association projections, and effective type filter; partial enumeration fails closed; complete non-empty enumerations emit a citable aggregate lane summary; and Phase 3 eval requires the named lane to complete plus exact, duplicate-free candidates, parse completeness, and exact cost reconciliation where configured. Prompt comparisons accept an alternate complete local prompt tree and record full content SHA-256 digests without exporting absolute workstation paths to Langfuse. Cost payloads with unsafe zero/missing totals or a returned subscription set different from the request are quarantined from reasoning, arithmetic, coverage, and cache admission. Current Phase 3 analyzer breadth remains intentionally narrow: `cost-summary` runs `orphan_public_ip` as an unassociated-resource review lane with calibrated weekly impact estimates from `pricing/azure-rate-card.json`, and loads `reasoner.v2` for cost-summary. Still pending: the bounded PostgreSQL/Logs/AKS/Cosmos/ACR service evidence packs, additional waste lanes, naming-pattern clustering, reasoner use of `prior_run_context` for continuity markers, uniform-drop freshness detection, Langfuse-backed prompt loading, Langfuse-as-source datasets, LLM-as-judge, human-review tooling, and configurable redaction.
