...
The repository currently ships `eval/phase-1.json` with four items, `eval/phase-3-waste.json` with the first orphan-public-IP waste-lane item, and `eval/phase-3-cost-reasoning.json` with two cost-judgment items. The latter checks that cost concentration without utilization evidence does not become a rightsizing claim, and that list-price public-IP exposure is reconciled against lower observed billed cost instead of being presented as realized savings. Automated scoring includes structural correctness, citation completeness, confidence consistency, read-only adherence, estimated-impact calibration, and waste-classification grounding; item-specific semantic expectations add non-vacuous, fail-closed checks for these two cost-judgment cases, including the exact billed components, exact lane range, and aggregate lane-summary citation. The CLI eval runner can run fully offline with fixture replay and mock credentials; when Langfuse credentials are present it can publish rubric and expectation scores, upsert dataset items, attach traces to a Dataset Run / Experiment, sweep multiple models, and select an alternate complete local prompt tree for baseline-versus-candidate experiments. Prompt labels and full content SHA-256 digests make those variants attributable even when the same path is edited in place. LLM-as-judge, human review, calibration reporting, continuity-specific eval items, and `continuity_grounded` remain planned.
...
- FR-17: Evaluation datasets should include billing-cache cases as the cache matures: cache-only usage-stable months, mixed cached-plus-live windows, not-yet-mature months that must not be cached, and corrupt or missing cache records that should degrade into explicit data-quality findings.
- FR-18: Cost-optimization datasets must include negative and contradictory examples that distinguish high spend from proven underutilization and observed billed cost from list-price exposure or realizable savings. Scenario-specific checks must fail closed when the evidence needed to evaluate the contract is absent, so omission cannot receive a vacuous pass.
