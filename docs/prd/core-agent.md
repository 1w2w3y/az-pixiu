...
- FR-21: The agent should be able to use local cached billing evidence for usage-stable full-month periods when the cache was populated from AMG-MCP and carries sufficient provenance. The agent must disclose cache use, must fall back or report a bounded gap when cache coverage is missing, and must not cache current or not-yet-stabilized periods or describe usage-stable data as invoice-finalized without an invoice-backed signal.
