...
The CLI currently exposes `pixiu analyze cost-surprise`, `pixiu analyze cost-summary`, `pixiu eval <dataset.json>`, and `pixiu diagnose`. Analyze supports explicit subscriptions, auto-discovery by top resource count, subscription-name filtering, sequential per-subscription batch execution, time windows, resource groups, resource type filters, fixture replay, playbook mode, mock reasoning mode (paired with `--use-playbook`, because the planner is not mocked), output directory selection, observability modes (`noop`, `memory`, `langfuse`, `ms-otel`), credential modes, and billing-access probes with cache. In sequential mode, discovery runs once and each selected subscription becomes an independent run with its own report, run artifact, and trace; planner and reasoner calls never overlap. Eval supports fixture-root and output-dir overrides, offline mock runs under the same playbook requirement, Langfuse publishing, dataset/experiment naming, model sweeps, and `--prompts-dir` for a complete alternate local prompt tree. The CLI prints the resolved prompt path locally; runs and eval metadata record stable prompt labels plus full content SHA-256 digests, while the absolute workstation path is not exported to Langfuse. It does not currently include `review`, `calibrate`, interactive follow-up, or package-managed installation workflows.
...
- FR-18: Evaluation commands must support selecting a complete alternate local prompt tree for baseline-versus-candidate experiments, print its resolved location locally, and record content-addressed prompt provenance in run and experiment metadata without exporting the absolute workstation path.
- FR-19: Analyze commands that select multiple subscriptions must offer an opt-in sequential mode that completes one independent subscription run before starting the next. The mode must bound each planner and reasoner input to one subscription, must not overlap model calls, and must preserve separate reports, run artifacts, traces, and run statuses rather than presenting the child outputs as a synthetic portfolio analysis. The batch process exit code must reflect the highest-precedence child failure.
