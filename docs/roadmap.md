...
## Phase 2.5 — Cross-run continuity foundations (shipped)

Phase 2.5 was the small, foundational increment that sat between the Langfuse-depth work of Phase 2 and the analyzer breadth of Phase 3. The motivation was that several Phase 3 features — flagging waste candidates that have not moved for N runs, recognizing a name pattern that recurred from a prior run, tracking whether a recommendation is new or carried forward — depend on the agent having any awareness of prior runs at all. The pre-Phase-2.5 agent was amnesiac: every `pixiu analyze` run started from zero. The [core agent PRD](prd/core-agent.md) Open Questions flagged this: "How should repeated recommendations be de-duplicated across runs or review periods?" and "What level of local state is needed to support comparison over time without creating a hidden data store requirement?"

Phase 2.5 answered those questions in the minimum way that unblocked Phase 3 without locking in a heavyweight state architecture. It introduced a `RunHistoryStore` interface with a default local-filesystem implementation that indexes the existing `runs/<run-id>/run.json` artefacts; a stable `recommendation_signature` field on the reasoner's output so the same recommendation can be recognized across runs even when the LLM rewrites its wording (v1 ships the LLM-emitted fallback slug per [`prompts/reasoner.v1.md`](../prompts/reasoner.v1.md) rule #15; the deterministic helper lives in [`src/reasoning/recommendation-signature.ts`](../src/reasoning/recommendation-signature.ts) and is called by Phase 3 lane code as those inputs land); and a `prior_run_context` evidence record that the orchestrator injects on every run for use by the reasoner. The substrate is deliberately a filesystem index over what the agent already persists — no SQLite, no remote store, no new infrastructure — so that offline operation continues to work end-to-end. The interface, however, is shaped so SQLite or a Langfuse Datasets backing can be swapped in later without changing the analyzer or the reasoner.

Phase 2.5 also landed the small "Run Quality" report section that promotes transport-level and freshness-related data-quality findings to a first-class report surface — independent of cross-run state but small enough to ship in the same increment — and the `--prior-run <run-id>` operator override (design §Gap 5 trade-off (c)) that injects a specific prior run into the reasoner's evidence regardless of scope match.

The full design lives in [cost-summary depth](design/cost-summary-depth.md) §Gap 5 and §Gap 6. The aim was that Phase 2.5 stay invisible to a casual operator — no new user-visible report sections beyond Run Quality — but that Phase 3's analyzer work can build directly on top of it. The project is now moving back into Phase 2 (Langfuse depth) and forward into Phase 3 (cost-summary depth and waste-detection lanes) against this substrate.
