...
This is the reasoning that supports treating AMG-MCP as a first-class part of the architecture rather than as an implementation detail. The [architecture principles](architecture-principles.md) document records the underlying preference for protocol-driven integration more generally. A separate [AMG-MCP capabilities](amg-mcp-capabilities.md) document records what the server currently exposes; it is intended as a snapshot of the surface, not as part of the architectural position.
